  ...
  Quintus.Input = function(Q) {
    ...
    Q.component('stepControls', {
      ...
      collision: function(col) {
        ...
        if (
          ...
          ((p.direction === 'up' && col.normalY === 1) ||
            (p.directions === 'down' && col.normalY === -1) ||
            (p.direction === 'left' && col.normalX === 1) ||
            (p.directions === 'right' && col.normalX === -1) ||
            (!col.objClassName && (col.normalX || col.normalY)))
        ) {
          p.stepping = false;
          p.x = p.origX;
          p.y = p.origY;
      ...
      step: function(dt) {
        ...
        if (p.stepDistance > p.tile_size) {
          p.stepDistance = p.tile_size;
        }
