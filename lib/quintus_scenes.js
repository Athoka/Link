...
var quintusScenes = function(Quintus) {
  ...
  Quintus.Scenes = function(Q) {
    ...
    */
    Q.Stage = Q.GameObject.extend({
      ...
      */
      ...
      /**
       Triggers range check and passes all the sprites and the dt for checking
      ...
      */
      triggerRange: function() {
        for (var i = this.items.length - 1; i >= 0; i--) {
          this.items[i].trigger('range.check', this.items);
        }
      },

      /**
       Returns every sprite in the scene
      ...
      */
      ...
      updateSprites: function(items, dt, isContainer) {
        ...
        for (var i = 0, len = items.length; i < len; i++) {
          ...
          if (
            !isContainer &&
            (item.p.visibleOnly && (!item.mark || item.mark < this.time))
          ) {
            continue;
          }
    ...
    */
    ...
    /**
     Changes the top scene to dst and activates the src one

     @method Q.swapScene
     @param {number} [src] - Position of the scene in the array to activate
     @param {number} [dst] - Position to move the current active stage to
     @for Quintus
     */
    Q.swapScene = function(src, dst) {
      if (!Q.stages[dst]) return;
      const last = Q.stages.length - 1;
      const tmp = Q.stages[last];
      Q.stages[last] = Q.stages[dst];
      Q.stages[src] = tmp;
      // Clean old pos
      Q.stages[dst].destroy();
      Q.stages[dst] = null;
    };
  };
};
