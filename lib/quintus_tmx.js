    ...
    Q._tmxProcessObjectLayer = function(stage, gidMap, tileProperties, layer) {
      ...
      for (var i = 0; i < objects.length; i++) {
        ...
        if (!overrideProperties) {
          throw 'Invalid TMX Object: missing properties for ID:' + id;
        }
        if (!overrideProperties['Class']) {
          throw 'Invalid TMX Object: missing Class for ID:' + id;
        }

        var className = overrideProperties['Class'];
        if (!className) {
          throw 'Invalid TMX Object Class: ' + className + ' ID:' + id;
        }

        var p = Q._extend({ x: x, y: y, name: name }, overrideProperties);
